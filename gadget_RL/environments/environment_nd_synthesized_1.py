...
import scipy
from VQEs import VQE_nd_synthesized_1 as vc
import os
import numpy as np
import copy
...
from ..utils.synthesized_gates import ryrx
...
class CircuitEnv():
    ...
    def __init__(self, conf, device):
        ...
        self.action_dict = dictionary_of_actions_nd_synthesized_1(self.num_qubits)
        ...
        stdout.flush()
        self.state_size = self.num_layers*self.num_qubits*(self.num_qubits+3+self.num_qubits+3+self.num_qubits)
    ...
    def step(self, action, train_flag = True) :
        ...
        ctrl1 = action[0] # CONTROL FOR CX
        targ1 = (action[0] + action[1]) % self.num_qubits # TARGET FOR CX
        ctrl2 = action[2] # CONTROL FOR RYRX
        targ2 = (action[2] + action[3]) % self.num_qubits # TARGET FOR RYRX
        ...
        # MOD RYRX GATE APPEND
        elif ctrl2 < self.num_qubits:
            next_state[gate_tensor][self.num_qubits+3+targ2][ctrl2] = 1
    ...
    def reset(self):
        ...
        state = torch.zeros((self.num_layers, (self.num_qubits+3+self.num_qubits)+(3+self.num_qubits), self.num_qubits))
    ...
    def make_circuit(self, thetas=None):
        ...
        for i in range(self.num_layers):
            """
            OLDS!
            cx_pos = np.where(state[i][0:self.num_qubits] == 1)
            rot_pos = np.where(state[i][self.num_qubits:2*self.num_qubits] == 1)
            ryrx_pos = np.where(state[i][2*self.num_qubits: 2*self.num_qubits+3] == 1)
            """
            ...
            # mod ryrx extration
            ryrx_pos = np.where(state[i][self.num_qubits+3: self.num_qubits+3+self.num_qubits] == 1)

            targ_cx = cx_pos[0]
            ctrl_cx = cx_pos[1]
            if len(ctrl_cx) != 0:
                for r in range(len(ctrl_cx)):
                    circuit.cx([ctrl_cx[r]], [targ_cx[r]])
            
            targ_ryrx = ryrx_pos[0]
            ctrl_ryrx = ryrx_pos[1]
            if len(ctrl_ryrx) != 0:
                for r in range(len(ctrl_ryrx)):
                    circuit.append(ryrx(thetas[i][targ_ryrx[r]+3][ctrl_ryrx[r]].item(), qubitrx=ctrl_ryrx[r], qubitry=targ_ryrx[r]), [ctrl_ryrx[r], targ_ryrx[r]])
    ...
    def scipy_optim(self, method, which_angles = [] ):
        ...
        thetas = state[:, self.num_qubits+3+self.num_qubits:]
        if self.decomposed:
            rot_pos = (state[:,self.num_qubits: self.num_qubits+3+self.num_qubits] == 1).nonzero( as_tuple = True )
        else:
            rot_pos = (state[:,self.num_qubits: self.num_qubits+3+self.num_qubits] == 1).nonzero( as_tuple = True )
        # print(rot_pos)
